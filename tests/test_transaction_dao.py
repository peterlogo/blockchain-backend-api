...
def test_get_all_transactions(db_engine):
    transaction_dao = TransactionDAO(db_engine)
    transactions = transaction_dao.get_all_transactions()
    assert transactions is not None
    
def test_get_transaction_by_tx_hash(db_engine):
    tx_hash = '0xb4563fa94ba33'
    transaction_dao = TransactionDAO(db_engine)
    transaction = transaction_dao.get_transaction_by_tx_hash(tx_hash)
    assert transaction is not None

def test_get_transaction_by_tx_hash_not_found(db_engine):
